  ...
  map(events, function(x) {
    x %>%
      ...
      map(~ mutate(.,
                   ...
                   defected_pass = get_pass_n(data = ., type = 'deflected'),
                   cross_pass = get_pass_n(data = ., type = 'cross'),
                   switch_pass = get_pass_n(data = ., type = 'switch'),
                   shot_assist = get_pass_n(data = ., type = 'shot_assist'),
                   cut_back = get_pass_n(data = ., type = 'cut_back'),
                   under_pressure = get_under_pressure_n(data = .),
                   yellow_card = get_cards_n(data = ., type = "Yellow Card"),
                   red_card = get_cards_n(data = ., type = "Red Card"),
